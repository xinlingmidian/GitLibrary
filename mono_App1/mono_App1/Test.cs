    ...
    public class Test
    {
        ...
        public static double matchingDegree(String matchfile, String inputfile, String outputfile)
        {
            ...
            try
            {
                ...
                Log.Error("test1", "jgks");
                //int len = matchfile.Length;
                double distance = Dtw.getDtw(matchfile, outputfile);
                //for (int i = 1; i < len; i++)
                //{
                //     distance = distance + Dtw.getDtw(matchfile[i].ToString(), outputfile);
                //}
                //distance = distance / len / 1.0;
                distance = distance / 1.0;
                var credit = ((50 - distance) / 50.0) * 100;
                Log.Error("test2", "jgks");
                if (credit < 80)
                    credit = 0;
                else if (credit > 100)
                    credit = 1;
            ...
            {
                Log.Error("testerror", ex.Message);
                return -1;
            }
